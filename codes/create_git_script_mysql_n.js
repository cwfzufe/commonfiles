...
db.query('SELECT stuId, stuName, gitUrl FROM tb_users WHERE gitUrl <> ""', function(err, data){

  var filename1='./gitclone_n.sh'
  fs.writeFileSync(filename1, '#! /bin/bash\ncd /var/www/html/homework\n\n', {flag: 'w'}, function(err){
    if (err) {
      console.log (err)
    }
  })
  var filename2='./gitpull_n.sh'
  ...
  for (var i=0; i < students.length; i++) {
    var ostr = `sudo git clone ${students[i].gitUrl} ${students[i].stuId}${idnames[students[i].stuId]}\n`
